...
- **About** (`src/i18n/strings.ts → about.body` + `about.cv.*`, both languages) — bio marked PLACEHOLDER; CV items in `[brackets]` are *to confirm* with her. Verified items (Underdogs #6 2015, Phases Magazine 2018, Dada Tapes 2016, Tuber 2017) have sources listed under the CV.
  Portrait: `public/assets/portrait/portrait-{1600,800,400}.webp` + `src/data/portrait.json` (size + blur).
  To replace: put the new file at `public/assets/portrait/portrait.jpg` and rerun the small Pillow
  snippet from git history (commit "Real portrait on About") — it writes the WebPs and json, then delete the jpg.
